...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (use_child_window)
    {
        ...
        table->OuterRect = table->InnerWindow->AABB();
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ...
    // Actual columns highlight/draw will be performed in EndTable() and not be affected.
    ImGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    if (is_visible)
    {
        ...
        if (draw_cell_bg_color)
        {
            ...
            {
                // As we draw the BG here we need to clip things (for layout we would not)
...
// Bg2 is used by Selectable (and possibly other widgets) to draw to the background.
...
// FIXME-TABLE: This is a mess, need to redesign how we draw borders (as some are also done in TableEndRow)
