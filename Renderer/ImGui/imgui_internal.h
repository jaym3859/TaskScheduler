...
struct IMGUI_API ImGuiWindow
{
    ...
    // The main 'OuterRect', omitted as a field, is window->AABB().
    ImRect                  OuterRectClipped;                   // == Window->AABB() just after setup in Begin(). == window->AABB() for root window.
...
public:
    ...
    ImRect      AABB() const            { return ImRect(Pos.x, Pos.y, Pos.x + Size.x, Pos.y + Size.y); }
